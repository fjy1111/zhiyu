...
def main():
    ...
    for key, typ in [('sample_id_removed','REMOVE_SAMPLE_ID'),('dataset_note_removed','REMOVE_DATASET_NOTE'),('hidden_prefix_removed','REMOVE_HIDDEN_PREFIX'),('temporary_marker_removed','REMOVE_TEMPORARY_MARKER'),('hard_negative_dataset_note_removed','REMOVE_HARD_NEGATIVE_DATASET_NOTE')]: audit[key]=sum(x['count'] for r in records for x in r.transformations if x['type']==typ)
    audit['other_transformations']=sorted({x['type'] for r in records for x in r.transformations if x['type'] not in {'REMOVE_SAMPLE_ID','REMOVE_DATASET_NOTE','REMOVE_HIDDEN_PREFIX','REMOVE_TEMPORARY_MARKER','REMOVE_HARD_NEGATIVE_DATASET_NOTE'}})
