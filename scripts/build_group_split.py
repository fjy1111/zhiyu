...
import yaml
from zhiyu.datasets.template_similarity import build_template_groups
...
def main():
    ...
    components,_=build_template_groups(rows,5,.8)
    groups={hashlib.sha256('|'.join(sorted(x['document_id'] for x in members)).encode()).hexdigest()[:16]:members for members in components}
